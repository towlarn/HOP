...
public class JInputBuffer<K extends Object, V extends Object> extends
		Buffer<K, V> implements InputCollector<K, V> {
	private static final Log LOG = LogFactory.getLog(JInputBuffer.class
			.getName());
	...
	class ShuffleRamManager implements RamManager {
		...
		public void update(int inputSize){
			size -= inputSize;
			fullSize = 0;
			numPendingRequests = 0;
			numClosed = 0;
		...
		public void closeInMemoryFile(int requestedSize) {
			synchronized (dataAvailable) {
				fullSize += requestedSize;
				++numClosed;
				LOG.debug("JInputBuffer: closeInMemoryFile. percent used = "
						+ getPercentUsed());
				dataAvailable.notify();
			}
		}
	 ...
	 */
	private final List<JInput> inputFilesInMemory = Collections
	...
	private Comparator<JInput> inputFileComparator = new Comparator<JInput>() {
	...
	private final SortedSet<JInput> inputFilesOnDisk = new TreeSet<JInput>(
			inputFileComparator);
	...
	public JInputBuffer(JobConf conf, Task task, Reporter reporter,
			Progress progress, Class<K> keyClass, Class<V> valClass,
		...
		this.maxInMemMerge = (int) Math.min(Runtime.getRuntime().maxMemory()
	...
	private void flush(long leaveBytes) throws IOException {
		...
		synchronized (inputFilesInMemory) {
			...
			mergeOutputSize = createInMemorySegments(inMemorySegments,
					leaveBytes);
		}

		Path outputPath = outputHandle.getInputFileForWrite(task.getTaskID(),
				taskid, spills++, mergeOutputSize);

		Writer writer = new Writer(conf, localFileSys, outputPath, keyClass,
				valClass, codec, null);

		RawKeyValueIterator rIter = null;
		try {
			int segments = inMemorySegments.size();
			LOG.info("Initiating in-memory merge with " + segments
					+ " segments... total size = " + mergeOutputSize);

			rIter = Merger.merge(conf, localFileSys, keyClass, valClass,
					inMemorySegments, inMemorySegments.size(), new Path(task
							.getTaskID().toString()), conf
							.getOutputKeyComparator(), reporter, null, null);
			...
			LOG.info(task.getTaskID() + " Merge of the " + segments
					+ " files in-memory complete." + " Local file is "
					+ outputPath + " of size "
		...
		LOG.info("FLUSH: Merged " + inMemorySegments.size() + " segments, "
				+ mergeOutputSize + " bytes to disk to satisfy "
				+ "reduce memory limit");
	}
	...
	//ivan: add new bucket to our reference list
	public void addNewBucket(){
		synchronized(this.lst_buckets){
			this.lst_buckets.add(new LinkedList<JInput>());
		}
	}

	@Override
	public synchronized boolean read(DataInputStream istream,
			OutputFile.Header header) throws IOException {
		...
		if (shuffleInMemory
			...
			Path filename = outputHandle.getInputFileForWrite(task.getTaskID(),
					taskid, spills++, decompressedLength);
	...
	private boolean shuffleInMemory(TaskID taskid, InputStream ins,
		...
		IFileInputStream checksumIn = new IFileInputStream(ins,
				compressedLength);
		ins = checksumIn;
		...
		LOG.debug("JBufferInput: copy compressed " + compressedLength
				+ " (decompressed " + decompressedLength + ") bytes from map "
				+ taskid);
		...
		try {
			...
			while (n > 0) {
				...
				reporter.progress();
				n = ins.read(shuffleData, bytesRead, shuffleData.length
						- bytesRead);
			}
	...
	private void shuffleToDisk(TaskID taskid, InputStream ins, Path filename,
		...
		try {
			...
			int n = ins.read(buf, 0,
					(int) Math.min(buf.length, mapOutputLength));
			while (n > 0) {
				bytesRead += n;
				shuffleClientMetrics.inputBytes(n);
				outs.write(buf, 0, n);
				...
				reporter.progress();
				n = ins.read(buf, 0,
						(int) Math.min(buf.length, mapOutputLength - bytesRead));
			}
...
//						.add(new Segment<K, V>(job, fs, file, codec, false));
		...
		if (0 != onDiskBytes) {
			...
			riter = Merger.merge(job, fs, keyClass, valClass, codec,
					finalSegments, ioSortFactor, numInMemSegments, tmpDir,
			...
			riter = Merger.merge(job, fs, keyClass, valClass, finalSegments,
					finalSegments.size(), tmpDir, comparator, reporter, null,
					null);
		}
	...
	/**
	 * An iterator that concurrent writes to a new spill file while client is
	 * reading. This is used for online aggregation. Basically, the task has
	 * called for an iterator while the input buffer is open. We take all
	 ...
	 * iterator while concurrent writing to a new spill file. When close is
	 * called the new spill file is registered as a new run, using the given
	 * taskid, and will be available during the next iterator call.
	 * 
	 */
	class RawKVIteratorWriter implements RawKeyValueIterator {
		...
		public RawKVIteratorWriter(RawKeyValueIterator riter, TaskID taskid,
				long bytes) throws IOException {
			this.riter = riter;
			this.taskid = taskid;
			this.outputPath = outputHandle.getInputFileForWrite(
					task.getTaskID(), taskid, spills++, bytes);
	...
	/*
	 * class RawKVIteratorReader extends IFile.Reader<K,V> { private final
	 * RawKeyValueIterator kvIter;
	 * 
	 * public RawKVIteratorReader(RawKeyValueIterator kvIter, long size) throws
	 * IOException { super(null, null, size, null, null); this.kvIter = kvIter;
	 * }
	 * 
	 * public boolean next(DataInputBuffer key, DataInputBuffer value) throws
	 * IOException { if (kvIter.next()) { final DataInputBuffer kb =
	 * kvIter.getKey(); final DataInputBuffer vb = kvIter.getValue(); final int
	 * kp = kb.getPosition(); final int klen = kb.getLength() - kp;
	 * key.reset(kb.getData(), kp, klen); final int vp = vb.getPosition(); final
	 * int vlen = vb.getLength() - vp; value.reset(vb.getData(), vp, vlen);
	 * bytesRead += klen + vlen; return true; } return false; }
	 * 
	 * public long getPosition() throws IOException { return bytesRead; }
	 * 
	 * public void close() throws IOException { kvIter.close(); } }
	 */
	...
	private class LocalFSMerger extends Thread {
		...
		public void run() {
			try {
				...
				while (!exit) {
					synchronized (inputFilesOnDisk) {
						while (!exit && inputFilesOnDisk.size() < mergeFactor) {
							LOG.info(task.getTaskID() + " Thread waiting: "
									+ getName() + ". Input files on disk "
									+ inputFilesOnDisk.size()
					...
					LOG.info(task.getTaskID() + "We have  "
							...
							+ files + " files");
					// 1. Prepare the list of files to be merged. This list is
					// prepared
					// using a list of map output files on disk. Currently we
					// merge
					...
					Path outputPath = lDirAlloc.getLocalPathForWrite(
							mapFiles.get(0).toString(), approxOutputSize, conf)
							.suffix(".merged");
					...
					try {
						iter = Merger.merge(conf, localFileSys, keyClass,
								valClass, codec,
								mapFiles.toArray(new Path[mapFiles.size()]),
								true, ioSortFactor, tmpDir,
								conf.getOutputKeyComparator(), reporter, null,
								null);
					...
					LOG.info(task.getTaskID() + " Finished merging "
							+ mapFiles.size()
